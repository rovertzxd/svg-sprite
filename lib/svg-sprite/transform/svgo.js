...
module.exports = function(shape, config, spriter, cb) {
    ...
    for (const preset of config.plugins) {
        ...
        if (preset.name !== undefined || preset.name === 'preset-default') {
            for (const key of Object.keys(preset)) {
                console.log(key);
                //config.plugins.preset.key.push(Boolean(spriter.config.svg.xmlDeclaration) ? 'removeXMLProcInst' : '');
                //config.plugins.preset.key.push(Boolean(spriter.config.svg.doctypeDeclaration) ? 'removeDoctype' : '');
            }
        }
    }

    //config.plugins[0].params.overrides = {
    //    ...config.plugins[0].params.overrides,
