...
SVGSpriteBase.prototype._buildCSSResources = function(files, cb) {
    ...
    const createResourceTask = (renderConfig, data, spriter, ext) => {
        return _cb => {
            const out = mustache.render(fs.readFileSync(renderConfig.template, 'utf-8'), data);
            if (out.length) {
                files[ext] = new Vinyl({
...
SVGSpriteBase.prototype._buildHTMLExample = function(files, cb) {
    if (this.config.example) {
        const out = mustache.render(fs.readFileSync(this.config.example.template, 'utf-8'), this.data);
        if (out.length) {
            files.example = new Vinyl({
