...
function SVGSpriterConfig(config) {
    ...
    (this.shape.spacing.padding || []).forEach(property => {
        let spacing;

        if (!Array.isArray(this.shape.spacing[property])) {
            spacing = Math.max(0, Number.parseInt(this.shape.spacing[property] || 0, 10));
            this.shape.spacing[property] = { top: spacing, right: spacing, bottom: spacing, left: spacing };
        } else {
            spacing = this.shape.spacing[property].map(n => Math.max(0, n));
 ...
 * @return {Object}
 */
SVGSpriterConfig.prototype._setupLogger = function(config) {
    this.log = '';

    if (Object.prototype.hasOwnProperty.call(config, 'log')) {
 ...
 * @return {Object}
 */
SVGSpriterConfig.prototype._parseMetaData = function() {
    if (Object.prototype.hasOwnProperty.call(this.shape, 'meta') && !_.isPlainObject(this.shape.meta)) {
 ...
 * @return {Object}
 */
SVGSpriterConfig.prototype._parseAlignmentData = function() {
    if (Object.prototype.hasOwnProperty.call(this.shape, 'align') && !_.isPlainObject(this.shape.align)) {
